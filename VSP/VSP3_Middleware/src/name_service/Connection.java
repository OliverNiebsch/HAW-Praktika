...
public class Connection extends Thread {
	...
	// private InputStreamReader reader = null; //new
	// InputStreamReader(socket.getInputStream());
	// private OutputStreamWriter writer = null; //new
	...
	*/
	...
	public void run() {
		...
		try {
			...
			OutputStreamWriter writer = new OutputStreamWriter(
					listeningSocket.getOutputStream());
			// while(!closeConnection){...}

			String inputMessage = reader.readLine();
			if (!(null == inputMessage)) {// Socket closed
				writeLine(handleInputMessage(inputMessage), writer);// SWITCH
																	// COMMANDS
			}
			reader.close();
			writer.close();
	...
	/*
	 * private String readLine() { if (reader == null) return null;
	 * 
	 * StringBuilder line = new StringBuilder(); char c = 'a'; boolean cr =
	 * false;
	 * 
	 * try { while (true) { c = (char) reader.read(); line.append(c);
	 * 
	 * if (cr && c == '\n') break;
	 * 
	 * if (c == '\r') { cr = true; } else { cr = false; } } } catch (IOException
	 * e) { e.printStackTrace(); }
	 * 
